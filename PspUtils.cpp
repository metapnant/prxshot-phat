...
bool PspUtils::button_locked = false;
...
bool PspUtils::isPressed(int buttons, int mask, unsigned int msecs) {
    // Check if the required button combination is currently being pressed
    if((buttons & mask) == mask) {
        
        // If the button is already locked from a previous screenshot, ignore it
        if(button_locked) {
            return false;
        }

        if(msecs == 0) {
            button_locked = true; // Lock it so it only fires once
            return true;
        } else {
            if(button_timeout == 0) {
        ...
        // The user let go of the button(s), or is pressing a different combo.
        // Reset the timeout and release the lock so they can take another picture!
        button_timeout = 0;
        button_locked = false;
    }
    return false;
