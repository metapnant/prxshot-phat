    ...
    if(mem_id < 0) {
        ...
        if(mem_id < 0 && api == PSP_INIT_KEYCONFIG_GAME) {
            ...
            if(!sceKernelVolatileMemTryLock(0, &ptr, &size)) {
                ...
                // If volatile also fails, try the User Memory Partition (Partition 2) as an absolute last resort
                kprintf("Allocating %i bytes from user memory\n", size);
                mem_id = sceKernelAllocPartitionMemory(2, GLB_BLK_NAME, PSP_SMEM_Low, size, NULL);
            }
        }
    }
